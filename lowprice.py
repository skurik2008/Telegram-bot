...
def realization(bot, data=None):
    """
    Функция первого порядка, содержит функции для пошаговой реализации команды /lowprice
    :param bot: бот
    ...
    def get_number_photo(message):
        """
        Функция второго порядка. Завершает работу команды /lowprice в случае отказа пользователя от показа фото отелей
        ...
        if message.text.lower() == 'да':
            ...
            try:
                current_hotels_list = utils.creat_hotel_list(data, command='lowprice')
                ...
                new_message = 'Извините! По вашим параметрам мне не удалось собрать информацию!' \
                              '\nПопробуйте изменить параметры для поиска!'
                utils.next_action_func(chat_bot=bot, user_message=message, bot_message=new_message)
                logging.exception(e)
            else:
                hotels_for_history = ', '.join(hotel_list_for_history)
                database.add_value(id_user=message.from_user.id, command='/lowprice', date=data.time_command, city=data.city_name,
                                   value=hotels_for_history)
    ...
    def get_answer_with_photos(message):
        ...
        и завершает выполнение команды /lowprice выводом ее результата
        :param message: блок данных по последнему входящему сообщению пользователя
        """
        try:
